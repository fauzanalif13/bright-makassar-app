...
import {
    createSingleFasilitator,
    createBatchFasilitators,
    updateFasilitator,
    deleteFasilitatorUser,
    toggleFasilitatorStatus,
    toggleFasilitatorBatchStatus,
    resetUserPassword,
    fetchFasilitatorUsers,
...
export default function AdminFasilitatorClient({ initialUsers, batchOptions }: Props) {
    ...
    async function handleToggleUser(user: PenggunaRow) {
        ...
        startTransition(async () => {
            const res = await toggleFasilitatorStatus(user.id, newStatus)
    ...
    async function handleDeleteConfirm() {
        ...
        startTransition(async () => {
            const res = await deleteFasilitatorUser(deletingUser.id)
            if (res.error) toast.error(res.error)
            else {
                toast.success(res.success!)
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p className="text-gray-500 dark:text-slate-400 text-sm mt-1">
                        Tambah, edit, dan kelola akun fasilitator BRIGHT Makassar.
                    </p>
                </div>
                <div className="flex items-center gap-3">
                    <button
                        onClick={() => setShowAddModal(true)}
                        className="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-bold text-white bg-gradient-to-r from-violet-500 to-purple-600 rounded-xl shadow-lg shadow-violet-500/25 hover:shadow-xl hover:from-violet-600 hover:to-purple-700 active:scale-[0.98] transition-all"
                    >
            ...
            <div className="flex flex-col sm:flex-row gap-3">
                <div className="relative flex-1">
                    ...
                    <input
                        ...
                        className="w-full pl-11 pr-4 py-3 bg-white dark:bg-slate-800/70 border border-gray-200 dark:border-slate-700/60 rounded-xl text-sm focus:ring-2 focus:ring-violet-500/30 focus:border-violet-500 dark:focus:border-violet-400 outline-none transition-all text-gray-800 dark:text-slate-200 placeholder:text-gray-400 dark:placeholder:text-slate-500"
                    />
                </div>
            </div>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="bg-gray-50 dark:bg-slate-900/40 border-b border-gray-100 dark:border-slate-700/60">
                                ...
                                <th className="text-left px-5 py-4 font-bold text-gray-500 dark:text-slate-400 uppercase text-xs tracking-wider">Batch / Angkatan (Asuhan)</th>
                            ...
                            {filteredUsers.length === 0 ? (
                                <tr>
                                    <td colSpan={6} className="px-5 py-12 text-center text-gray-400 dark:text-slate-500">
                                        ...
                                        <p className="font-semibold">Tidak ada fasilitator ditemukan</p>
                                ...
                                <tr key={u.id} className="hover:bg-gray-50/60 dark:hover:bg-slate-700/20 transition-colors">
                                    <td className="px-5 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-9 h-9 bg-gradient-to-br from-violet-500 to-purple-600 rounded-xl flex items-center justify-center text-white font-bold text-xs shadow-sm shrink-0">
                                    ...
                                    <td className="px-5 py-4">
                                        <span className="inline-flex items-center px-2.5 py-1 bg-violet-50 dark:bg-violet-900/20 text-violet-700 dark:text-violet-400 text-xs font-bold rounded-lg">
                                    ...
                                    <td className="px-5 py-4">
                                        <div className="flex items-center justify-center gap-1">
                                            <button
                                                ...
                                                className="p-2 rounded-lg text-gray-400 dark:text-slate-500 hover:text-violet-600 dark:hover:text-violet-400 hover:bg-violet-50 dark:hover:bg-violet-900/15 transition-all"
                                                title="Edit"
                                            >
            ...
            <DeleteConfirmModal
                isOpen={!!deletingUser}
                count={1}
                label={`fasilitator "${deletingUser?.name || ''}"`}
    ...
    async function refreshAll() {
        try {
            const freshUsers = await fetchFasilitatorUsers()
            setUsers(freshUsers)
        } catch { /* swallow */ }
    }
}
...
// ═══════════════════════════════════════════════════════════════════════
const inputCls = "w-full px-4 py-3 bg-gray-50 dark:bg-slate-900/60 border border-gray-200 dark:border-slate-700/60 rounded-xl text-sm focus:ring-2 focus:ring-violet-500/30 focus:border-violet-500 dark:focus:border-violet-400 outline-none text-gray-800 dark:text-slate-200 placeholder:text-gray-400 dark:placeholder:text-slate-500 transition-all"
...
function AddUserModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
    ...
    function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        startTransition(async () => {
            const res = await createSingleFasilitator(formData)
    ...
    return (
        <ModalShell onClose={onClose}>
            <ModalHeader icon={<UserPlus className="w-5 h-5 text-white" />} gradient="from-violet-500 to-purple-600" title="Tambah Fasilitator Baru" onClose={onClose} />
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
                ...
                <div>
                    <label className={labelCls}>Gender Asuhan</label>
                ...
                <SubmitButton isPending={isPending} icon={<Plus className="w-4 h-4" />} label="Tambah Fasilitator" pendingLabel="Menyimpan..." gradient="from-violet-500 to-purple-600" shadowColor="shadow-violet-500/25" />
            </form>
        </ModalShell>
    )
}
...
function BatchUserModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
    ...
    function addRow() { setRows(prev => [...prev, { name: '', email: '', password: '' }]) }
    ...
    function handleSubmit(e: React.FormEvent) {
        ...
        if (validRows.length === 0) { setError('Isi minimal satu baris fasilitator.'); return }

        startTransition(async () => {
            const res = await createBatchFasilitators(validRows, {
                angkatan: sharedAngkatan,
                gender: sharedGender,
                batch: sharedBatch,
            })
            if (res.created > 0) toast.success(`${res.created} fasilitator berhasil ditambahkan!`)
    ...
    return (
        <ModalShell onClose={onClose} wide>
            <ModalHeader icon={<Users className="w-5 h-5 text-white" />} gradient="from-violet-500 to-indigo-600" title="Tambah Batch Fasilitator" subtitle="Tambahkan beberapa fasilitator sekaligus" onClose={onClose} />
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
                {error && <ErrorBanner message={error} />}
                ...
                <div className="bg-gray-50 dark:bg-slate-900/40 rounded-xl p-4 border border-gray-100 dark:border-slate-700/60">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                        <div>
                            <label className="block text-xs font-bold text-gray-600 dark:text-slate-400 mb-1">Batch Asuhan</label>
                            <input value={sharedBatch} onChange={e => handleSharedBatchChange(e.target.value)} className={inputCls} placeholder="10" />
                        </div>
                        <div>
                            <label className="block text-xs font-bold text-gray-600 dark:text-slate-400 mb-1">Angkatan Asuhan</label>
                            <input value={sharedAngkatan} onChange={e => handleSharedAngkatanChange(e.target.value)} className={inputCls} placeholder="2024" />
                        </div>
                        <div>
                            <label className="block text-xs font-bold text-gray-600 dark:text-slate-400 mb-1">Gender Asuhan</label>
                ...
                <div className="space-y-3">
                    <div className="flex items-center justify-between">
                        ...
                        <button type="button" onClick={addRow} className="inline-flex items-center gap-1 text-xs font-bold text-violet-600 dark:text-violet-400 hover:underline">
                ...
                <SubmitButton isPending={isPending} icon={<Users className="w-4 h-4" />} label={`Tambah ${rows.filter(r => r.name && r.email && r.password).length} Pengguna`} pendingLabel="Memproses..." gradient="from-violet-500 to-indigo-600" shadowColor="shadow-violet-500/25" />
            </form>
        </ModalShell>
    )
}
...
function EditUserModal({ user, onClose, onUpdated }: { user: PenggunaRow; onClose: () => void; onUpdated: (u: PenggunaRow) => void }) {
    ...
    function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        const formData = new FormData(e.currentTarget)
        startTransition(async () => {
            const res = await updateFasilitator(user.id, formData)
            if (res.error) setError(res.error)
            else {
                toast.success(res.success!)
                onUpdated({
    ...
    return (
        <ModalShell onClose={onClose}>
            <ModalHeader icon={<Edit3 className="w-5 h-5 text-white" />} gradient="from-amber-400 to-orange-500" title="Edit Fasilitator" onClose={onClose} />
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
                {error && <ErrorBanner message={error} />}
