...
export default function PrestasiPage() {
    ...
    async function handleDelete(ri: number[]) { setDeleteModal({ isOpen: false, rowIndices: [] }); setIsDeleting(true); setEntries(prev => prev.filter(e => !ri.includes(e.rowIndex))); setSelectedRows(new Set()); const r = await deletePrestasiEntries(ri); setIsDeleting(false); if (r.error) { toast.error(r.error); fetchEntries(true) } else toast.success(r.success!) }
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden">
                    ...
                    <form action={handleSubmit} className="p-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div><label htmlFor="tanggal" className="flex items-center gap-1.5 text-xs font-bold text-gray-700 dark:text-slate-200 mb-1.5"><CalendarDays className="w-3.5 h-3.5 text-amber-500" /> Tanggal <span className="text-red-400">*</span></label><input type="text" id="tanggal" name="tanggal" placeholder="2024" required className="w-full px-3.5 py-2.5 bg-gray-50 dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-xl text-sm text-gray-800 dark:text-slate-100 placeholder:text-gray-400 focus:ring-2 focus:ring-amber-400/40 outline-none transition-all" /></div>
