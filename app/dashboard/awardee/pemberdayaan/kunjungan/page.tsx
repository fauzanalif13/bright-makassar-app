...
export default function KunjunganPage() {
    ...
    async function handleDelete(ri: number[]) { setIsDeleting(true); setEntries(prev => prev.filter(e => !ri.includes(e.rowIndex))); setSelectedRows(new Set()); const r = await deleteKunjunganEntries(ri); setIsDeleting(false); setDeleteModal({ isOpen: false, rowIndices: [] }); if (r.error) { toast.error(r.error); fetchEntries(true) } else toast.success(r.success!) }
